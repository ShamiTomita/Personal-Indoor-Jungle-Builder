  ...
  def call
    ...
    IndoorJungle::Plant.all.each do |plant|
      ...
      if temp.include?(plant) && light.include?(plant) && attention.include?(plant)
        plants << plant
        #switch = "one" #all three pass
      #elsif plants.empty? && temp.include?(plant) && light.include?(plant) && !attention.include?(plant)
      #  plants << plant
      #  switch = "two" #temp and light pass
      ######
      #elsif plants.empty? && light.include?(plant) && attention.include?(plant) && !temp.include?(plant)
      #plants << plant
      #switch = "three" # light and attention pass
      ######
      #else plants.empty? && temp.include?(plant) && attention.include?(plant) && !light.include?(plant)
      #plants << plant
      #switch = "four" # temp and attention pass
      ######
      end
    end
    #if switch == "one"
      ...
      sleep(2)
    #elsif switch == "two"
      #puts "Your home enviornment is ideal but with some habit tweaking, we believe you could grow:"
    #elsif switch == "three"
      #puts "Your habits and light sources seem ideal, but if you were to raise your home temp you could ideally grow"
    #elsif switch == "four"
      #puts "Your home temperature and your habits seem ideal, if you were to include a grow light or another suplemental light source, you could ideally grow:"
    #end
